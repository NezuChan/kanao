...
import type { ClientOptions, FrameworkClient } from "../Lib/FrameworkClient.js";
...
export type FrameworkPluginHookEntry<T = FrameworkPluginAsyncHook | FrameworkPluginHook> = {
    hook: T;
    type: PluginHook;
    name?: string;
};
...
export class PluginManager {
    ...
    public registerHook(hook: FrameworkPluginAsyncHook | FrameworkPluginHook, type: PluginHook, name?: string): this {
        if (typeof hook !== "function") throw new TypeError(`The provided hook ${name === undefined ? "" : `(${name}) `}is not a function`);
    ...
    public * values(hook?: PluginHook): Generator<FrameworkPluginHookEntry, void> {
        for (const plugin of this.registry) {
            if ((hook !== undefined) && plugin.type !== hook) continue;
            yield plugin;
        }
    }
}
