...
export class GuildMemberUpdateListener extends Listener {
    public async run(payload: { data: GatewayGuildMemberUpdateDispatch }): Promise<void> {
        ...
        if (Util.optionalEnv<boolean>("STATE_USER", "true") || cast<APIUser>(JSON.parse(botUser ?? "{ id: null }")).id === payload.data.d.user.id) await this.container.gateway.cache.users.set(payload.data.d.user.id, payload.data.d.user);
