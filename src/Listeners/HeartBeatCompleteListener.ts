...
export class ReadyListener extends Listener {
    ...
    public async run(payload: { shard: WebSocketShard; shardId: number; data: { latency: number } }): Promise<void> {
        ...
        const status = JSON.parse(await this.store.redis.get(`${clientId}:gateway_shard_status:${payload.shardId}`) ?? "{}") as object | { latency: number; status: number; startAt: number };
