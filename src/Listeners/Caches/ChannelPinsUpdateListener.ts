...
export class ChannelPinsUpdateListener extends Listener {
    ...
    public async run(payload: { data: GatewayChannelPinsUpdateDispatch }): Promise<void> {
        if ("guild_id" in payload.data.d && payload.data.d.guild_id) {
            const guild_channel = await this.store.redis.get(`${clientId}:${RedisKey.CHANNEL_KEY}:${payload.data.d.guild_id}:${payload.data.d.channel_id}`);
            if (guild_channel) {
                ...
                await this.store.redis.set(`${clientId}:${RedisKey.CHANNEL_KEY}:${payload.data.d.guild_id}:${payload.data.d.channel_id}`, JSON.stringify(channel));
            }
        } else {
            const non_guild_channel = await this.store.redis.get(`${clientId}:${RedisKey.CHANNEL_KEY}:${payload.data.d.channel_id}`);
            if (non_guild_channel) {
                const channel = JSON.parse(non_guild_channel) as APITextBasedChannel<TextChannelType>;
                channel.last_pin_timestamp = payload.data.d.last_pin_timestamp;
                await this.store.redis.set(`${clientId}:${RedisKey.CHANNEL_KEY}:${payload.data.d.channel_id}`, JSON.stringify(channel));
            }
        }
    }
}
