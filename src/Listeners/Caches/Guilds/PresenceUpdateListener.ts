...
export class GuildMemberUpdateListener extends Listener {
    ...
    public async run(payload: { data: GatewayPresenceUpdate; shardId: number }): Promise<void> {
        if (statePresences) {
            ...
            await this.store.redis.sadd(GenKey(`${RedisKey.PRESENCE_KEY}${RedisKey.KEYS_SUFFIX}`, payload.data.guild_id), GenKey(`${RedisKey.PRESENCE_KEY}`, payload.data.user.id, payload.data.guild_id));
        }
