...
export class ChannelDeleteListener extends Listener {
    public async run(payload: { data: GatewayChannelDeleteDispatch }): Promise<void> {
        ...
        if ("guild_id" in payload.data.d && payload.data.d.guild_id) await this.container.gateway.cache.channels.delete(`${payload.data.d.guild_id}:${payload.data.d.id}`);
        else await this.container.gateway.cache.channels.delete(payload.data.d.id);
