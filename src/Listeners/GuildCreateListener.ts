    ...
    public async run(payload: { data: GatewayGuildCreateDispatch }): Promise<void> {
        ...
        for (const member of payload.data.d.members) {
            if (Util.optionalEnv<boolean>("STATE_USER", "true")) {
                await this.container.gateway.redis.sadd(this.container.gateway.genKey(RedisKey.USER_KEY, true), `${payload.data.d.id}:${member.user!.id}`);
                await this.container.gateway.cache.users.set(member.user!.id, member.user);
            }
            if (Util.optionalEnv<boolean>("STATE_MEMBER", "true")) {
                await this.container.gateway.redis.sadd(this.container.gateway.genKey(RedisKey.MEMBER_KEY, true), `${payload.data.d.id}:${member.user!.id}`);
        ...
        for (const role of payload.data.d.roles) {
            if (Util.optionalEnv<boolean>("STATE_ROLE", "true")) {
                await this.container.gateway.redis.sadd(this.container.gateway.genKey(RedisKey.ROLE_KEY, true), `${payload.data.d.id}:${role.id}`);
        ...
        for (const voiceState of payload.data.d.voice_states) {
            if (Util.optionalEnv<boolean>("STATE_VOICE", "true")) {
                await this.container.gateway.redis.sadd(this.container.gateway.genKey(RedisKey.VOICE_KEY, true), `${payload.data.d.id}:${voiceState.user_id}`);
        ...
        for (const channel of payload.data.d.channels) {
            if (Util.optionalEnv<boolean>("STATE_CHANNEL", "true")) {
                await this.container.gateway.redis.sadd(this.container.gateway.genKey(RedisKey.CHANNEL_KEY, true), `${payload.data.d.id}:${channel.id}`);
        ...
        for (const emoji of payload.data.d.emojis) {
            if (emoji.id) {
                if (Util.optionalEnv<boolean>("STATE_EMOJI", "true")) {
                    await this.container.gateway.redis.sadd(this.container.gateway.genKey(RedisKey.EMOJI_KEY, true), `${payload.data.d.id}:${emoji.id}`);
        ...
        for (const presence of payload.data.d.presences) {
            if (Util.optionalEnv<boolean>("STATE_PRESENCE", "true")) {
                await this.container.gateway.redis.sadd(this.container.gateway.genKey(RedisKey.PRESENCE_KEY, true), `${payload.data.d.id}:${presence.user.id}`);
        ...
        await this.container.gateway.redis.sadd(this.container.gateway.genKey(RedisKey.GUILD_KEY, true), payload.data.d.id);
